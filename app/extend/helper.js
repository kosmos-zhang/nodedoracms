...
module.exports = {
  ...
  clearRedisByType(str, cacheKey) {
    ...
    const currentKey = this.ctx.session_secret + cacheKey + str;
  ...
  sendEmail(sysConfigs, key, obj = {}, callBack = () => {}) {
    ...
    if (key == emailTypeKey.email_findPsd) {
      toEmail = obj.email;
      const oldLink = obj.password + '$' + obj.email + '$' + this.ctx.session_secret;
      const newLink = this.encrypt(oldLink, this.app.config.encrypt_key);
  ...
  checkTempUnzipSuccess(targetForder) {
    return new Promise((resolve, reject) => {
      ...
      const USERS_DIR = system_template_forder + targetForder + '/users';
      const TWOSTAGEDEFAULT_DIR = system_template_forder + targetForder + '/2-stage-default';
