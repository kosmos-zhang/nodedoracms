...
class HomeController extends Controller {
  ...
  getCateOrDetailTemp(defaultTempItems, contentTemp = '', type) {
    ...
    if (!_.isEmpty(contentTemp)) {
      ...
      const defaultItem = _.filter(defaultTempItems, temp => {
  ...
  async getSiteMapPage() {
    ...
    // 类别地图
    const allCategories = await this.ctx.helper.reqJsonData('contentCategory/getList');
  ...
  async getPageData(that) {
    ...
    // 获取当前模板信息
    const defaultTemp = await ctx.helper.reqJsonData('contentTemplate/getDefaultTempInfo');
    ...
    // 针对分类页和内容详情页动态添加meta
    const defaultTempItems = defaultTemp.items;
    if (!_.isEmpty(pageData.siteInfo)) {
      const siteDomain = pageData.siteInfo.configs.siteDomain;
      ...
      if (ctx.pageType === 'index') { // 首页
        ...
        const cateName = _.isEmpty(pageData.cateInfo) ? '' : (' | ' + pageData.cateInfo.name);
        ...
        if (!_.isEmpty(pageData.documentInfo)) {
          ...
          const parentCateTemp = pageData.documentInfo.categories[0].contentTemp;
